...
gcc -v
# 运行脚本
# nohup srun -p I950 -N 1 -c 2 --mem-per-cpu=100G ./mfcompress.sh > /public/home/jd_sunhui/genCompressor/LRCB/result/mfcompress_16.out &
echo "1 设置实验参数，为了避免错误，使用绝对路径."
ResultDir="/public/home/jd_sunhui/genCompressor/LRCB/result"
Algorithm="mfcompress"
threads=16
# 将时间戳转换为秒
...
for SourceDataDir in $F_NAME; do
  ...
  (/bin/time -v -p /public/home/jd_sunhui/genCompressor/LRCB/MFCompress-linux64-1.01/MFCompressC ${FileBaseName}.fasta) >${FileBaseName}_${threads}_com.log 2>&1

  echo "统计压缩信息"
  # xm需要保存序列长度索引恢复序列，使用readsNum*32 bits计算
  CompressedFileSize=$(ls -lah --block-size=1 ${FileBaseName}.fasta.mfc | awk '/^[-d]/ {print $5}')
  ...
  echo "de-compression..."
  (/bin/time -v -p /public/home/jd_sunhui/genCompressor/LRCB/MFCompress-linux64-1.01/MFCompressD ${FileBaseName}.fasta.mfc) >${FileBaseName}_${threads}_decom.log 2>&1
  echo "统计压缩信息"
  ...
  echo "3.5 清除脏文件"
  echo "cmp ${FileBaseName}.fasta.mfc.d ${FileBaseName}.fasta"
  cmp ${FileBaseName}.fasta.mfc.d ${FileBaseName}.fasta
  tail -c 100 ${FileBaseName}.fasta.mfc.d
  tail -c 100 ${FileBaseName}.fasta
  rm -rf ${FileBaseName}.reads
  rm -rf ${FileBaseName}.fastq
  rm -rf ${FileBaseName}.fasta
  rm -rf ${FileBaseName}.fasta.mfc.d
  rm -rf ${FileBaseName}.fasta.mfc
