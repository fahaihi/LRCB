#!/bin/bash
# 运行脚本
# nohup srun -p gpu1 -N 1 -c 16 ./enano.sh > /public/home/jd_sunhui/genCompressor/LRCB/result/enano_16.out &
...
for SourceDataDir in $F_NAME; do
  ...
  echo "compression..."
  (/bin/time -v -p enano -c -t ${threads} ${FileBaseName}.fastq ${FileBaseName}.enano) >${FileBaseName}_${threads}_com.log 2>&1
  echo "统计压缩信息"
  CompressedFileSize=$(grep "Bases" ${FileBaseName}_${threads}_com.log | awk '{print $4}')
  ...
  echo "de-compression..."
  (/bin/time -v -p enano -d -t ${threads} ${FileBaseName}.enano ${FileBaseName}.enano.fastq) >${FileBaseName}_${threads}_decom.log 2>&1
  echo "统计压缩信息"
  ...
  rm -rf ${FileBaseName}.enano
  rm -rf ${FileBaseName}.enano.fastq
...
#!/bin/bash
# nohup srun -p gpu1 -c 8 /public/home/jd_sunhui/genCompressor/longReads/script/enano.sh ERR5396170 8 > result_ERR5396170/ERR5396170.enano &
scriptPath="/public/home/jd_sunhui/genCompressor/longReads/script/"
file=$1
threads=$2
echo "*******************************************************************************************"
echo "a script files for enano test compression and de_compression"
# colord输出的单位为B
echo "compression"
/bin/time -v -p enano -c -t ${threads} ${file}.fastq ${file}.enano
echo "*******************************************************************************************"
echo "de-compression"
/bin/time -v -p enano -d -t ${threads} ${file}.enano ${file}.enano.fastq