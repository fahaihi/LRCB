...
gcc -v
# 运行脚本
# nohup srun -p gpu1 -N 1 -c 16 ./spring.sh > /public/home/jd_sunhui/genCompressor/LRCB/result/spring_16.out &
...
for SourceDataDir in $F_NAME; do
  ...
  echo "compression..."
  (/bin/time -v -p spring -c -l -i ${FileBaseName}.fastq --no-quality --no-ids -o ${FileBaseName}.spring -t ${threads}) >${FileBaseName}_${threads}_com.log 2>&1
  echo "统计压缩信息"
  CompressedFileSize=$(grep "Reads:" ${FileBaseName}_${threads}_com.log | awk '{print $2}')
  ...
  echo "de-compression..."
  (/bin/time -v -p spring -d -l -i ${FileBaseName}.spring -o ${FileBaseName}.spring.reads -t ${threads}) >${FileBaseName}_${threads}_decom.log 2>&1
  echo "统计压缩信息"
  ...
  rm -rf ${FileBaseName}.spring
  rm -rf ${FileBaseName}.spring.reads
...
exit 0
#!/bin/bash
# nohup srun -p gpu1 -c 8 /public/home/jd_sunhui/genCompressor/longReads/script/spring.sh ERR5396170 8 > result_ERR5396170/ERR5396170.spring &
scriptPath="/public/home/jd_sunhui/genCompressor/longReads/script/"
file=$1
threads=$2
echo "*******************************************************************************************"
echo "a script files for spring test compression and de_compression"
# spring输出的单位为B
echo "compression"
/bin/time -v -p spring -c -l -i ${file}.fastq --no-quality --no-ids -o ${file}.spring -t ${threads}
echo "*******************************************************************************************"
echo "de-compression"
/bin/time -v -p spring -d -l -i ${file}.spring -o ${file}.spring.reads -t ${threads}