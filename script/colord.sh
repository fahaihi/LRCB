#!/bin/bash
# 运行脚本
# nohup srun -p gpu1 -N 1 -c 16 --mem-per-cpu=10G ./colord.sh > /public/home/jd_sunhui/genCompressor/LRCB/result/colord_plus_16.out &
echo "1 设置实验参数，为了避免错误，使用绝对路径."
ResultDir="/public/home/jd_sunhui/genCompressor/LRCB/result"
Algorithm="colord_plus"
threads=16
# 将时间戳转换为秒
...
for SourceDataDir in $F_NAME; do
  ...
  ((index++))
  echo "index : D${index}"
  if [[ $index -eq 7 || $index -eq 8 || $index -eq 11 || $index -eq 17 || $index -eq 20 || $index -eq 23 || $index -eq 27 || $index -eq 29 || $index -eq 30 || $index -eq 31 ]]; then
    echo "compress-pbraw"
    (/bin/time -v -p colord compress-pbraw -q org -p ratio -t ${threads} ${FileBaseName}.fastq ${FileBaseName}.colord) >${FileBaseName}_${threads}_com.log 2>&1
  fi
  if [[ $index -eq 18 || $index -eq 21 || $index -eq 24 || $index -eq 25 || $index -eq 26 ]]; then
    echo "compress-pbhifi"
    (/bin/time -v -p colord compress-pbhifi -q org -p ratio -t ${threads} ${FileBaseName}.fastq ${FileBaseName}.colord) >${FileBaseName}_${threads}_com.log 2>&1
  fi
  if [[ $index -eq 1 || $index -eq 2 || $index -eq 3 || $index -eq 4 || $index -eq 5 || $index -eq 6 || $index -eq 9 || $index -eq 10 || $index -eq 12 || $index -eq 13 || $index -eq 14 || $index -eq 15 || $index -eq 16 || $index -eq 19 || $index -eq 22 || $index -eq 28 ]]; then
    echo "compress-ont"
