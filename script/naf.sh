...
gcc -v
# 运行脚本
# nohup srun -p gpu1 -N 1 -c 2 --mem-per-cpu=30G ./naf.sh > /public/home/jd_sunhui/genCompressor/LRCB/result/naf_16.out &
...
for SourceDataDir in $F_NAME; do
  ...
  (/bin/time -v -p ennaf ${FileBaseName}.fasta --temp-dir ./ -o ${FileBaseName}.naf) >${FileBaseName}_${threads}_com.log 2>&1

  echo "统计压缩信息"
  # xm需要保存序列长度索引恢复序列，使用readsNum*32 bits计算
  CompressedFileSize=$(ls -lah --block-size=1 ${FileBaseName}.naf | awk '/^[-d]/ {print $5}')
  ...
  echo "de-compression..."
  (/bin/time -v -p unnaf ${FileBaseName}.naf -o ${FileBaseName}.naf.fasta) >${FileBaseName}_${threads}_decom.log 2>&1
  echo "统计压缩信息"
  ...
  echo "3.5 清除脏文件"
  echo "cmp ${FileBaseName}.naf.fasta ${FileBaseName}.fasta"
  cmp ${FileBaseName}.naf.fasta ${FileBaseName}.fasta
  tail -c 100 ${FileBaseName}.naf.fasta
  tail -c 100 ${FileBaseName}.fasta
  rm -rf ${FileBaseName}.reads
  rm -rf ${FileBaseName}.fastq
  rm -rf ${FileBaseName}.fasta
  rm -rf ${FileBaseName}.naf.fasta
  rm -rf ${FileBaseName}.naf
