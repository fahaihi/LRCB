#!/bin/bash
# 运行脚本
# nohup srun -p I950 -N 1 -c 16 --mem-per-cpu=15G ./nanospring.sh >> /public/home/jd_sunhui/genCompressor/LRCB/result/nanospring_16.out &
echo "1 设置实验参数，为了避免错误，使用绝对路径."
ResultDir="/public/home/jd_sunhui/genCompressor/LRCB/result"
Algorithm="nanospring_supp2"
threads=16
# 将时间戳转换为秒
...
echo "3 执行算法压缩及解压缩操作"
F_NAME=$(cat /public/home/jd_sunhui/genCompressor/LRCB/script/F_Name_NanoSpring.txt | tr -d '\r')
for SourceDataDir in $F_NAME; do
  ...
  echo "compression..."
  (/bin/time -v -p NanoSpring -c -i ${FileBaseName}.fastq -o ${FileBaseName}.nanospring -t ${threads}) >${FileBaseName}_${threads}_com.log 2>&1
  echo "统计压缩信息"
  CompressedFileSize=$(ls -lah --block-size=1 ${FileBaseName}.nanospring | awk '/^[-d]/ {print $5}') #以字节为单位显示原始文件大小
  ...
  echo "de-compression..."
  (/bin/time -v -p NanoSpring -d -i ${FileBaseName}.nanospring -o ${FileBaseName}.nanospring.reads -t ${threads}) >${FileBaseName}_${threads}_decom.log 2>&1
  echo "统计压缩信息"
  ...
  rm -rf ${FileBaseName}.nanospring
  rm -rf ${FileBaseName}.nanospring.reads
...
exit 0
#!/bin/bash
# nohup srun -p gpu1 -c 8 /public/home/jd_sunhui/genCompressor/longReads/script/nanospring.sh ERR5396170 8 > result_ERR5396170/ERR5396170.nanospring &
scriptPath="/public/home/jd_sunhui/genCompressor/longReads/script/"
file=$1
threads=$2
echo "*******************************************************************************************"
echo "a script files for nanospring test compression and de_compression"
# spring输出的单位为B
echo "compression"
/bin/time -v -p NanoSpring -c -i ${file}.fastq -o ${file}.nanospring -t ${threads}
#ls -l --block-size=1 ${file}.nanospring
echo "*******************************************************************************************"
echo "de-compression"
/bin/time -v -p NanoSpring -d -i ${file}.nanospring -o ${file}.nanospring.reads -t ${threads}