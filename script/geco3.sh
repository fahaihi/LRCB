...
gcc -v
# 运行脚本
# nohup srun -p gpu1 -N 1 -c 1 --mem-per-cpu=100G ./geco3.sh >> /public/home/jd_sunhui/genCompressor/LRCB/result/geco3_16.out &
echo "1 设置实验参数，为了避免错误，使用绝对路径."
ResultDir="/public/home/jd_sunhui/genCompressor/LRCB/result"
Algorithm="geco3_supp2"
threads=16
# 将时间戳转换为秒
...
echo "3 执行算法压缩及解压缩操作"
F_NAME=$(cat /public/home/jd_sunhui/genCompressor/LRCB/script/F_Name_GeCo3.txt | tr -d '\r')
for SourceDataDir in $F_NAME; do
  ...
  (/bin/time -v -p GeCo3 -l 5 -lr 0.03 -hs 40 ${FileBaseName}.reads) >${FileBaseName}_${threads}_com.log 2>&1
  echo "统计压缩信息"
  # geco3 不支持字符N，以及需要额外保存序列长度，因此我们使用32bits记录序列长度，使用32bits记录N所在位置
