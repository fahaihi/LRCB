...
gcc -v
# 运行脚本
# nohup srun -p gpu1 -N 1 -c 16 --mem-per-cpu=15G ./nuht.sh >> /public/home/jd_sunhui/genCompressor/LRCB/result/nuht_16.out &
echo "1 设置实验参数，为了避免错误，使用绝对路径."
ResultDir="/public/home/jd_sunhui/genCompressor/LRCB/result"
Algorithm="nuht_supp2"
threads=16
# 将时间戳转换为秒
...
echo "3 执行算法压缩及解压缩操作"
F_NAME=$(cat /public/home/jd_sunhui/genCompressor/LRCB/script/F_Name_NUHT.txt | tr -d '\r')
for SourceDataDir in $F_NAME; do
  ...
  (/bin/time -v -p /public/home/jd_sunhui/genCompressor/LRCB/Non-Greedy-Unbalanced-Huffman-Tree-Compressor-for-single-and-multi-fasta-files/Linux/NUHT_Compress ${FileBaseName}.fasta) >${FileBaseName}_${threads}_com.log 2>&1

  echo "统计压缩信息"
  # xm需要保存序列长度索引恢复序列，使用readsNum*32 bits计算
  CompressedFileSize=$(ls -lah --block-size=1 ${FileBaseName}.nuht | awk '/^[-d]/ {print $5}')
  ...
  echo "de-compression..."
  (/bin/time -v -p /public/home/jd_sunhui/genCompressor/LRCB/Non-Greedy-Unbalanced-Huffman-Tree-Compressor-for-single-and-multi-fasta-files/Linux/NUHT_Decompress ${FileBaseName}.nuht) >${FileBaseName}_${threads}_decom.log 2>&1
  echo "统计压缩信息"
  ...
  echo "3.5 清除脏文件"
  echo "cmp ${FileBaseName}_.fasta ${FileBaseName}.fasta"
  cmp ${FileBaseName}_.fasta ${FileBaseName}.fasta
  tail -c 100 ${FileBaseName}_.fasta
  tail -c 100 ${FileBaseName}.fasta
  rm -rf ${FileBaseName}.reads
  rm -rf ${FileBaseName}.fastq
  rm -rf ${FileBaseName}.fasta
  rm -rf ${FileBaseName}_.fasta
