#!/bin/bash
# 运行脚本
# nohup srun -p gpu1 -N 1 -c 16 ./pigz.sh > /public/home/jd_sunhui/genCompressor/LRCB/result/pigz_16.out &
...
for SourceDataDir in $F_NAME; do
  ...
  echo "compression..."
  (/bin/time -v -p pigz -c -11 -p "${threads}" "${FileBaseName}.reads" >${FileBaseName}.gz) >${FileBaseName}_${threads}_com.log 2>&1
  echo "统计压缩信息"
  CompressedFileSize=$(ls -lah --block-size=1 ${FileBaseName}.gz | awk '/^[-d]/ {print $5}')
  ...
  echo "de-compression..."
 # (/bin/time -v -p pigz -dc -p ${threads} ${FileBaseName}.gz > ${FileBaseName}.de_reads) > ${FileBaseName}_${threads}_decom.log 2>&1
  (/bin/time -v -p pigz -dc -p ${threads} ${FileBaseName}.gz > ${FileBaseName}.de_reads) > ${FileBaseName}_${threads}_decom.log 2>&1
  echo "统计压缩信息"
  ...
  rm -rf ${FileBaseName}.gz
  rm -rf ${FileBaseName}.de_reads
