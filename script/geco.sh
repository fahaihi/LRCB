...
gcc -v
# 运行脚本
# nohup srun -p gpu1 -N 1 -c 1 --mem-per-cpu=100G ./geco.sh > /public/home/jd_sunhui/genCompressor/LRCB/result/geco_16.out &
...
for SourceDataDir in $F_NAME; do
  ...
  (/bin/time -v -p /public/home/jd_sunhui/genCompressor/LRCB/geco/src/GeCo -l 5 ${FileBaseName}.reads) >${FileBaseName}_${threads}_com.log 2>&1
  echo "统计压缩信息"
  # geco 不支持字符N，以及需要额外保存序列长度，因此我们使用32bits记录序列长度，使用32bits记录N所在位置
  ...
  echo "de-compression..."
  (/bin/time -v -p /public/home/jd_sunhui/genCompressor/LRCB/geco/src/GeDe ${FileBaseName}.reads.co) >${FileBaseName}_${threads}_decom.log 2>&1
  echo "统计压缩信息"
  ...
  echo "3.5 清除脏文件"
  cmp ${FileBaseName}.reads ${FileBaseName}.reads.de

  tail -c 100 ${FileBaseName}.reads
  tail -c 100 ${FileBaseName}.reads.de
