...
gcc -v
# 运行脚本
# nohup srun -p gpu1 -N 1 -c 1 --mem-per-cpu=100G ./geco2.sh > /public/home/jd_sunhui/genCompressor/LRCB/result/geco2_16.out &
...
for SourceDataDir in $F_NAME; do
  ...
  (/bin/time -v -p GeCo2 -v -l 5 ${FileBaseName}.reads) >${FileBaseName}_${threads}_com.log 2>&1
  echo "统计压缩信息"
  # geco2 不支持字符N，以及需要额外保存序列长度，因此我们使用32bits记录序列长度，使用32bits记录N所在位置
  ...
  echo "de-compression..."
  (/bin/time -v -p GeDe2 ${FileBaseName}.reads.co) >${FileBaseName}_${threads}_decom.log 2>&1
  echo "统计压缩信息"
